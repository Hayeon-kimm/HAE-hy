...
class hae(nn.Module):
    ...
    def forward(self, x, y = None, batch_size=4, resize=True, latent_mask=None, input_code=False, randomize_noise=True,
        ...
        if not input_feature:
            ...
            ocodes = codes # [8, 18, 512], w_i
            feature = self.mlp_encoder(ocodes) # [8, 512] z_Ri
            feature_reshape = torch.flatten(feature, start_dim=1) # [8, 512]
            feature_dist = self.hyperbolic_linear(feature_reshape) # [8, 512]
        ...
        feature_euc = gmath.logmap0(feature_dist, k=torch.tensor(-1.)) # [8, 512]
        feature_euc = self.mlp_decoder(feature_euc) # [8,18,512]
        codes = feature_euc
