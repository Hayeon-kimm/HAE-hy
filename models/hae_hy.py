...
class hae(nn.Module):
    ...
    def __init__(self, opts):
        ...
        self.fc_layer_enc = nn.Linear(4 * 32 * 32, 512)
        self.fc_layer_dec = nn.Linear(512, 4 * 32 * 32)
    ...
    def forward(self, x, y = None, batch_size=4, resize=True, latent_mask=None, input_code=False, randomize_noise=True,
        ...
        if not input_feature:
            if input_code:
                ...
                with torch.no_grad():
                    codes = self.autoencoder.encode(x).latent_dist.sample() * 0.18215 # [8, 4, 32, 32]
                # normalize with respect to the center of an average face(이 부분을 일단 빼야할듯? avg를 구할 순 없으니..)
                # if self.opts.start_from_latent_avg:
                #     if self.opts.learn_in_w:
                #         codes = codes + self.latent_avg.repeat(codes.shape[0], 1)
                #     else: # HERE
                #         import pdb; pdb.set_trace()
                #         codes = codes + self.latent_avg.repeat(codes.shape[0], 1, 1) # [8,1,512] 를 더한거

            ocodes = codes
            flattened_ocodes = ocodes.view(ocodes.size(0), -1)
            feature = self.fc_layer_enc(flattened_ocodes)
        ...
        feature_euc = self.fc_layer_dec(feature_euc)
        feature_euc = feature_euc.view(ocodes.size(0), 4, 32, 32)
        codes = feature_euc
        ...
        with torch.no_grad() :
            images = self.autoencoder.decode(codes).sample
            
        if resize:
            images = self.face_pool(images)
        result_latent = codes # ADD
        if return_latents:
